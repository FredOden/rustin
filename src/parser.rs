...
impl Parsed {
    fn _new(at: usize, count: usize, p__: Vec<Parsed>, val: String) -> Parsed {
        Parsed {
            at,
            count,
            p__,
...
impl Parser {
    pub fn new(grammar_json: String, tokens: Vec<lex::Token>) -> Parser {
        ...
        for (k, v) in &p.grammar.rules {
            ...
            for i in v.keys() {
                r.push((*i).to_string());
                p.forbidden_rules.push(ForbiddenRule::new((*i).to_string()));
            }
    ...
    fn insert_cache(&self, try_rule: &String, at: usize, opt_parsed: Option<Parsed>) {
        self.cache
            .borrow_mut()
            .insert(CacheKey::K(try_rule.clone(), at), opt_parsed);
    }

    fn find_cache(&self, try_rule: &String, at: usize) -> Option<Parsed> {
        match &self.cache.borrow().get(&CacheKey::K(try_rule.clone(), at)) {
            Some(x) => {
                let op = *x;
                match op {
                    None => {
    ...
    fn is_broken(&self, try_rule: &String, at: usize) -> bool {
        match self.broken.borrow().get(&CacheKey::K(try_rule.clone(), at)) {
            Some(b) => {
                return *b;
            }
            None => {
    ...
    fn set_broken(&self, try_rule: &String, at: usize) {
        self.broken
            .borrow_mut()
            .insert(CacheKey::K(try_rule.clone(), at), true);
    }
    ...
    pub fn parse(&self, name: String, at_start: usize) -> Option<Parsed> {
        self.inc_depth();
        ...
        let mut at = at_start;
        let vok = binding.iter_mut();
        for try_rules in vok {
            'rulesLoop: for try_rule in try_rules.iter() {
                ...
                'syntax: for i_element in 0..syntax.len() {
                    ...
                    let mandatory = element.len() > EXPECT.len()
                        && element[0..EXPECT.len()].cmp(EXPECT) == Ordering::Equal;
                    if mandatory {
                        element = element[EXPECT.len()..].to_string();
                        /*
                        println!(
                            "{}::!!!! {try_rule}:: mandatory::{mandatory} element::<{element}>",
                            self.depth()
                        );
                        */
                    }
                    ...
                    if self.tokens[at + i_element].token().cmp(&element) != Ordering::Equal {
                        if element[0..1].cmp("&") == Ordering::Equal {
                            ...
                            let opt_parsed =
                                self.parse(sub_rule.to_string(), at + i_element);
                            if let Some(fr) = self.find_forbidden(try_rule) {
                                fr.remove_at(at);
                            }
                            if let Some(parsed) = opt_parsed {
                                ...
                                if mandatory {
                                    eprintln!(
                                        "!!!! {name}:: Syntax error at {} {element} expected",
                                        self.line_col(at + i_element)
                                    );
                                }
                                break 'syntax;
                            }
                    ...
                    if i_element == count - 1 {
                        ...
                        /*
                        println!(
                            "{}::@@@@ Matched {try_rule} {i_element} & at {} ==> {val} {}",
                            self.depth(),
                            self.line_col(at + i_element),
                            self.tokens[at + i_element].string()
                        );
                        */
                        let op = Some(Parsed {
                            at,
        ...
        if terminal {
            eprintln!(
                "{}::!!!! {name}:: Syntax error at {} unexpexted \"{}\"",
                self.depth(),
                self.line_col(at),
                self.tokens[at].string()
            );
        }
